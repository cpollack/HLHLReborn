...
function GetHLHLVer: WideString;	// 返回 HLHL 版本
function HexToInt(HexStr: String): Int64;
procedure WindowSetText(myhwnd: HWND; text: String);
function WindowGetText(myhwnd: HWND): WideString; // 得到窗口内容的文字
...
function IsNeiyao(Name: WideString): Boolean;  // 判断是否是内药
...
function CanCreateWG:String; // 返回值为空串的时候，说明OK
...
begin
  ...
  tmpWin := HLInfoList.GlobalHL.ItemOfWindowTitle('详细属性');
  if tmpWin <> Nil
  then
  begin
...
var
  ...
  tmpX, tmpY: DWORD;
begin
  lpBuffer := GetUserEnvState;	// 获得用户当前状态

  if lpBuffer <> UserEnvBattle then	// 并非战斗状态，允许移动
  begin
...
// 获取当前地图信息
begin
  Result := GameMaps.ItemOfMapID(ReadCurrMapID);	// 获取指定标识的地图信息
end;
...
var
  tmpWin: TWindowInfo;
begin
  if HLInfoList.GlobalHL.LocateToPlayWindow(tmpWin) = -1 then Exit; // 没找到
...
begin
  Result := False;
  if (name = '回灵粉') or (name = '珍珠丹') or (name = '还神丹') or (name = '琼露丹') then
    Result := True;
end;
...
procedure InitNPCs;
// 初始化 NPC 信息，包括地图编号、NPC类型、名称、满足条件
begin
  ...
  //NPCs[Ord(ShuiCheng_Forge)].SatisfiedCond := '锻造店 - 物品分类';
