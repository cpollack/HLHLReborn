...
type
  ...
  public
    ...
    procedure MakeStoveInstruction; // 没有处理宠物的情况，宠物的ATTR为$4d4
  end;

  TStoveStuffToBeChozen1 = record
    Stuff: TItem;
    ChozenBy: TComboBox; // 被谁Chozen了，如果是Nil，则没有被Chozen
  end;
...
begin
  ...
  Label6.Caption := '共有' + IntToStr(lstWindows.Count) + '个窗口';
end;
...
begin
  Memo1.Lines.Clear;
  memo1.lines.Add('用户：' + HLInfoList.GlobalHL.UserName
    + ', 绰号：' + HLInfoList.GlobalHL.UserNick
    + ', 配偶：' + HLInfoList.GlobalHL.UserSpouse);
...
begin
  ...
  begin
    ...
    tmpString := 'Step ' + IntToStr(i + 1) + ': 从[' + tmpMap.Name + ']的';
    if tmpNode.InHL_X = -1 then // 要过传送点
    begin
      tmpTrans := GameMaps.TransportList.Items[tmpNode.InHL_Y];
      tmpString := tmpString + '传送点[' + IntToStr(tmpTrans.ID) + ']';
    end
    else
    ...
    tmpString := tmpString + '到[' + tmpMap.Name + ']';
...
begin
  ...
  GetCurrPosXY(X, Y);
  Label17.Caption := '现在' + lpMapName + '[' + IntToStr(MapID)
    + ']，坐标为' + IntToStr(X) + ',' + IntToStr(Y);
...
begin
  ...
  begin
    ...
    ListBox6.Items.Add('传送点[' + IntToStr(tmpTransport.ID) + ']: '
  ...
  StringGrid1.Cells[2, 0] := '制作人';
  StringGrid1.Cells[3, 0] := '类型';
  StringGrid1.Cells[4, 0] := '+血';
  StringGrid1.Cells[5, 0] := '+内';
  StringGrid1.Cells[6, 0] := '+攻';
  StringGrid1.Cells[7, 0] := '+防';
  StringGrid1.Cells[8, 0] := '+敏';

  ComboBoxStoveMainRoom.Clear;
  ComboBoxStoveMainRoom.Items.Add('--未选择--');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if tmpStoveStuffToBeChozen.ChozenBy=tmpComboBox then tmpComboBox.ItemIndex:=tmpindex; // 本来就是这个ComboBox选择的
        tmpindex:=tmpindex+1;
      end;
    end;
  end;
...
procedure TFormDevelop.MakeStoveInstruction; // 没有处理宠物的情况，宠物的ATTR为$4d4
var
  i: Integer;
  tmpComboBox: TComboBox;
...
begin
  ...
  ListBoxItemsInStove.Items.Add('主炼位置：'+IntToStr(StoveInstruction.MainStuffPos));
  for i := 0 to 7 do
  begin
...
procedure TFormDevelop.InitStoveStuffsToBeChozen; // 先不处理炼化宠物情况
{var
  i, j: integer;      }
begin
{
  ...
  begin
    if (UserItems.Items[i].Name='※乾坤袋') or (UserItems.Items[i].Name='※百宝囊') then continue;
...
begin
  ...
  ListBoxCurrInstruction.Items.Add('主炼位置：' + IntToStr(MainStuffPos));
...
begin
  ...
  // Transaction 0
  tmpTransaction := ThisWork.AddTransaction('设置武功参数');
  tmpTransaction.AddStep(ActSetWGAttr, '黑暗星辰', '2', '500');
  tmpTransaction.AddStep(ActSetWGDisp, '7', '15', '14');
  tmpTransaction.AddStep(ActOpenItemWindow, '', '', '');

  // Transaction 1
  tmpTransaction := ThisWork.AddTransaction('准备买药');
  tmpPurpose := tmpTransaction.AddPurpose;
  tmpPurpose.Allows.Add('物品,回灵粉,天晴无名氏,1');
  tmpTransaction.AddStep(ActCallNPC, '水城药店大夫', '', '');
  tmpTransaction.AddStep(ActBuyStuff, '回灵粉', '天晴无名氏', '');
  tmpTransaction.AddStep(ActQuitShop, '', '', '');

  // Transaction 2
  tmpTransaction := ThisWork.AddTransaction('测试有无药品');
  tmpPurpose:= tmpTransaction.AddPurpose;
  tmpPurpose.Allows.Add('物品,回灵粉,天晴无名氏,1');
  tmpTransaction.AddStep(ActJumpToTransN, '1', '', '');

  // Transaction 3
  tmpTransaction := ThisWork.AddTransaction('吃药＋创招；创满就不创了');
  tmpPurpose := tmpTransaction.AddPurpose;
  tmpPurpose.Allows.Add('人物,武功数,※限制数');
  tmpTransaction.AddStep(ActUseItem, '回灵粉', '天晴无名氏', '');
  tmpTransaction.AddStep(ActCreateWG, '', '', '');
  tmpTransaction.AddStep(ActJumpToTransN, '2', '', '');

  // Transaction 4
  tmpTransaction := ThisWork.AddTransaction('删招');
//  tmpTransaction.AddStep(ActDeleteWGs, '5', '保留容易', '');
  tmpTransaction.AddStep(ActDeleteWGs, '5', '※保留系数', '2.3');

  // Transaction 5
  tmpTransaction := ThisWork.AddTransaction('测试删招后，招数是否还是满的');
  tmpPurpose := tmpTransaction.AddPurpose;
  tmpPurpose.Allows.Add('人物,武功数,※限制数');
  tmpTransaction.AddStep(ActJumpToTransN, '3', '', '');

  // Transaction 6
  tmpTransaction := ThisWork.AddTransaction('结束');
